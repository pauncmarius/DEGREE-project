...
namespace FCUnirea.Persistance.Data
{
    public class FCUnireaDbContext : DbContext
    {
        ...
        private static void SeedDatabase(ModelBuilder modelBuilder)
        {
            ...
            {
                new Players() { Id = 1, PlayerName = "Alex Popescu", Position = Position.Atacker, BirthDate = new DateTime(1998, 5, 20) },
                new Players() { Id = 2, PlayerName = "Mihai Ionescu", Position = Position.Midfielder, BirthDate = new DateTime(2000, 8, 10) }
            });
            ...
            {
                new Seats() { Id = 1, SeatType = SeatType.Vip, SeatName = "A1", SeatPrice = 150, Reserved = false },
                new Seats() { Id = 2, SeatType = SeatType.Standard, SeatName = "B2", SeatPrice = 50, Reserved = false }
            });

            modelBuilder.Entity<Games>().HasData(new List<Games>()
            {
                new Games() { Id = 1, GameDate = DateTime.UtcNow, HomeTeamScore = 2, AwayTeamScore = 1, TicketsSold = 5000 }
            });

            modelBuilder.Entity<Tickets>().HasData(new List<Tickets>()
            {
                new Tickets() { Id = 1, DateReservation = DateTime.UtcNow },
            });

            modelBuilder.Entity<TeamStatistics>().HasData(new List<TeamStatistics>()
            {
                new TeamStatistics() { Id = 1, GamesPlayed = 15, TotalWins = 10, TotalLosses = 2, TotalDraws = 3, GoalsScored = 30, GoalsConceded = 15, TotalPoints = 33 }
            });

            modelBuilder.Entity<PlayerStatisticsPerCompetition>().HasData(new List<PlayerStatisticsPerCompetition>()
            {
                new PlayerStatisticsPerCompetition() { Id = 1, Goals = 10, Assists = 5, Saves = 0, YellowCards = 2, RedCards = 0, MinutesPlayed = 1200 }
            });

            modelBuilder.Entity<PlayerStatisticsPerGame>().HasData(new List<PlayerStatisticsPerGame>()
            {
                new PlayerStatisticsPerGame() { Id = 1, Goals = 2, Assists = 1, PassesCompleted = 30, Saves = 0, YellowCards = 0, RedCards = 0, MinutesPlayed = 90 }
            });

            modelBuilder.Entity<News>().HasData(new List<News>()
            {
                new News() { Id = 1, Title = "Victorie mare pentru FC Unirea!", Text = "FC Unirea a câștigat cu 2-1!", CreatedAt = DateTime.UtcNow }
            });
